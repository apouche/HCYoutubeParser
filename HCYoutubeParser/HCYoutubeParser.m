...
+ (NSString *)youtubeIDFromYoutubeURL:(NSURL *)youtubeURL {
    ...
    // first attempt take that video id parameters
    NSDictionary* params = [youtubeURL dictionaryForQueryString];
    youtubeID = params[@"v"];

    // second attempt extract the id from path
    if (youtubeID == nil) {
        NSRange range = [youtubeURL.path rangeOfString:@"[\\w_]{6,}$" options:NSRegularExpressionSearch];
        if (range.location != NSNotFound) {
            youtubeID = [youtubeURL.path substringWithRange:range];
        }
    }
